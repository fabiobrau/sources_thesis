...
import sys
from convex_adversarial import robust_loss, DualNetwork
sys.path.append("../")
...
ax.axis([0, 1, 0, 1])
plt.axis('off')
#plt.savefig('./images/2Drobpoints'+str(m)+'.png')
...
#plt.savefig('./images/2Drobpoints'+str(m)+'testing.png')
for i in range(m):
    ax.add_patch(patches.Rectangle((X[i, 0]-r, X[i, 1]-r), 2*r, 2*r, fill=False))
plt.savefig('./images/2Drobpoints'+str(m)+'tested.png')
